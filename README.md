# Quant Trader — UK Stock-Ranking Research (Phase 1–4)
...
stocks will outperform their peers over the following few months, in a way
that would survive real transaction costs inside a Trading 212 Stocks & Shares
ISA? Built end-to-end on free data (`yfinance`), with £0/month running cost.

**Headline result (Phase 4):** across seven model families — historical mean,
momentum, ridge, elastic net, logistic regression, random forest, and gradient
boosting — none showed predictive value that survived robustness checks at the
3-month or 1-month horizon. The apparent edge from the tree models turned out
to be overfitting to a single four-month market regime (COVID recovery,
Aug–Nov 2020), not a stable signal. That negative result, and the process that
produced it, is the actual point of this repository — see
[`PHASE4_CONCLUSION_3M.md`](PHASE4_CONCLUSION_3M.md).

## Why this project is worth reading, even though it "failed"
...
- **Frozen hyperparameter grids and decision tables**, fixed before the
  ...
  back.
- **A 27-date locked test set that was never touched.** Every experiment in
  this repository ran against train/validation data only. No result in this
  repository is a confirmatory test-set number.
...
Phase 4 is the headline: it's where the pre-registration discipline, the
robustness gates, and the honest negative result all come together. Phases
1–3 are the infrastructure that makes Phase 4's result trustworthy — without
a leakage-free, validated data pipeline and backtesting engine, a "no
predictive value" conclusion wouldn't mean anything.
...
src/
  ...
  ml/           Phase 4: features, targets, walk-forward splits, models
tests/          122 tests: unit, synthetic-recovery, and leakage-control
```
...
python3 -m pytest tests/ -q          # 122 tests, no network or database required
```

The database and raw price/membership data are not included in this
repository (see Data below). To regenerate them and reproduce the Phase 4
results:
...
`PHASE4_CONCLUSION_3M.md` come directly from these reports and are cited by
experiment ID for traceability.
...
No model in this repository has ever been used to place a live or paper
trade. Phase 4's conclusion is that the V1 feature set showed no robust
predictive value at the horizons tested — the honest reading of that is
"don't trade this," not "needs more tuning until it works." Nothing here
should be read as a recommendation to buy, sell, or hold any security.
